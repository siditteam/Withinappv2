...
import { inquiries, inquiryGuidance } from "./inquiryData";
...
export default function InquirySession() {
  ...
  const completePath = `/inquiry/${id}/complete`;
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setSeconds((prev) => {
        if (prev <= 1) {
          clearInterval(timer);
          // Timer naturally ended — initiate exit
  ...
  return (
    ...
    >
      {/* Ambient background */}
      <SessionBackground color={inquiry.color} />
      ...
      >
        ...
        <div className="relative w-full max-w-[320px] mb-8" style={{ minHeight: 60 }}>
          <AnimatePresence mode="wait">
            {(phase === "instruction" || phase === "closing") && !exiting && (
              ...
              >
                &ldquo;{inquiry.instruction}&rdquo;
              </motion.p>
            )}
          </AnimatePresence>
        </div>
        ...
        >
          <BreathingOrb color={inquiry.color} theme="dark" paused={isPaused || exiting} />
        </motion.div>
